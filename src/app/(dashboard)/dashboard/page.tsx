...
// This should be your app/dashboard/page.tsx
export default function DashboardPage() {
  const [currentTime, setCurrentTime] = useState(new Date());
  const clientName = 'John Anderson'; // This could come from auth context or params
        ...
        <div className="space-y-4">
          {recentTransactions.map((transaction) => (
            <div key={transaction.id} className="flex items-center justify-between py-3 border-b border-slate-100 last:border-b-0">
              ...
              <p className="font-semibent text-slate-900">{transaction.amount}</p>
            </div>
          ))}
        </div>
      </div>
